  ...
  <v-main class="mt-n16">
    <v-parallax class="index_parallax" src="/images/home/banner.webp">
      <div class="text-center mt-16 first-box">
        ...
        <v-btn v-if="status === 'unauthenticated'" prepend-icon="mdi-login-variant" rounded="lg" size="x-large" @click="$router.push('/login')">Login Now!</v-btn>
        <v-btn v-else prepend-icon="mdi-login-variant" rounded="lg" size="x-large" @click="data.role === 'USER' ? $router.push('/reservation') : scrollTo('management_features')">
