  ...
  <v-main class="management_main">
      ...
      <v-card :width="mobile ? 'auto' : '400px'">
        ...
        <v-form
          ...
          <v-card-text>
            ...
            <v-text-field v-model="menuPrice" :prefix="'฿'" :rules="[priceRule]" label="Price" min="1" oninput="validity.valid || (value=1)" type="number"></v-text-field>
            <v-text-field v-model="menuImgUrl" :rules="[urlValidator]" label="Image URL"></v-text-field>
            <v-select v-model="menuCategoryID" :items="menuCategory" hide-details item-title="c_name" item-value="c_id" label="Category"></v-select>
            <v-btn append-icon="mdi-shape" class="mt-4" color="warning" variant="text" @click="manageCategory = true">Manage Category</v-btn>
          </v-card-text>
          <v-card-actions>
            <v-btn :append-icon="menuDialogMode == 0 ? 'mdi-plus' : 'mdi-content-save'" :disabled="!menuName" :text="menuDialogMode == 0 ? 'Add' : 'Save'" color="success" type="submit"></v-btn>
            <v-btn
              color="primary"
              @click="
                () => {
    ...
    <div class="main_container management_container mx-auto blur-effect">
      ...
      <v-sheet class="mt-8 ma-md-8 ma-sm-5" rounded="lg">
        ...
        <v-data-table :density="mobile ? 'compact' : 'comfortable'" :group-by="groupBy" :headers="dtHeaders" :items="dtData" :loading="dtLoading" :search="dtSearch" class="elevation-1" fixed-header height="60vh" item-value="m_id" items-per-page="-1" sticky>
          <template v-slot:top>
                  ...
                  <v-row>
                    ...
                    <v-col cols="12" md="4" sm="6">
                      ...
                      <v-card class="text-center" elevation="4" ripple>
                        <v-img :src="item.m_img ? item.m_img : '/images/img-coming-soon.webp'" aspect="16/9" cover height="300">
