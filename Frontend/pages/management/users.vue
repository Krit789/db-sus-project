...
<template>
  <v-main class="management_main">
    ...
    <v-dialog v-model="userAction" :fullscreen="mobile" :width="mobile ? '100%' : '500px'">
    ...
    <v-dialog v-model="userPWResetDialog" :fullscreen="mobile" :width="mobile ? '100%' : '500px'">
      <v-card :width="mobile ? '100%' : '500px'">
        ...
        <v-card-actions>
          <v-btn append-icon="mdi-check" color="success"
                 @click="() => {loadingDialog = true; resetPassword(userID); userID = 0;}">Confirm
          </v-btn>
          <v-btn append-icon="mdi-cancel" color="error" @click="userPWResetDialog = false">Cancel</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <v-dialog v-model="newUserPWDialog" :fullscreen="mobile" :width="mobile ? '100%' : '500px'">
      <v-card :width="mobile ? '100%' : '500px'">
        <v-card-title>New Password</v-card-title>
        <v-card-text>
          <p>This is the new password for '{{ firstName + " " + lastName }}' ID: {{ userID }}. This dialog will only
            show up <b>once</b>! Make sure to keep it safe.</p>
          <br>
