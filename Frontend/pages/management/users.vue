                ...
                <v-data-table v-model:items-per-page="itemsPerPage" :headers="dtHeaders" :items="dtData" :loading="dtLoading" :search="dtSearch" class="elevation-1" item-value="user_id">
                    ...
                    <template v-slot:item="{ internalItem, item, toggleExpand, isExpanded }">
                        ...
                        >
                            ...
                            <td class="text-start td-hover">{{ item.email }}</td>
                            ...
                            <td class="text-start td-hover">
                                <v-tooltip location="top">
                                    <template v-slot:activator="{ props }">
                                        <v-icon v-bind="props">{{ item.status == "ACTIVE" ? "mdi-check" : item.status == "ACTIVE" ? "mdi-close" : "mdi-help" }}</v-icon>
                                    </template>
                                    <span>{{ item.status == "ACTIVE" ? "Active" : item.status == "ACTIVE" ? "Suspended" : item.status }}</span>
                                </v-tooltip>
                            </td>
                        </tr>
                    </template>
                                ...
                                <v-container>
                                    ...
                                    <v-row>
                                        <v-col>
                                            ...
                                            <v-btn variant="text" prepend-icon="mdi-gavel" class="mr-5" color="error">Suspend User</v-btn>
