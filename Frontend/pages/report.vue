  ...
  <v-main class="justify-center report_main">
    ...
    <v-dialog v-model="selectReport" :fullscreen="mobile" :width="mobile ? '100%' : '900px'" persistent="">
      <v-card class="" style="overflow: initial; z-index: initial">
        ...
        <v-card-item>
          <v-no-ssr>
            <v-data-table v-model="selectedDT" :density="mobile ? 'compact' : 'comfortable'" :headers="dtHeaders" :items="dtData" :loading="dtLoading" :search="dtSearch" class="elevation-1" height="40vh" item-value="l_id" items-per-page="-1" select-strategy="page" show-expand show-select sticky>
              <template v-slot:top>
                <v-row>
                  <v-col>
                    <h4>Begin</h4>
        ...
        <v-card-actions>
          ...
          <v-btn :disabled="selectedDT.length <= 0" color="success" prepend-icon="mdi-chart-timeline-variant-shimmer" @click="loadReportData(DateTime.fromISO(reportBeginTime), DateTime.fromISO(reportEndTime), selectedDT)">Generate Report</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <div v-show="!selectReport || salesChartData.datasets[0].data.length !== 0" class="main_container mx-auto blur-effect py-4 px-2 mt-8 account_container justify-center">
      ...
      <v-btn class="ml-10" color="info" prepend-icon="mdi-select-multiple-marker" variant="tonal" @click="selectReport = true">Re-select Branches</v-btn>
      <v-container>
        <v-row class="mt-6">
        ...
        <v-row class="py-10 px-5">
          <v-col class="box-container">
            ...
            <v-sheet class="mt-5 ma-md-8 ma-xs-1 text-center bg-transparent" rounded="lg">
              <v-table :density="mobile ? 'compact' : 'comfortable'" class="mx-3 bg-transparent" fixed-header height="300px">
                <thead>
                  <tr>
